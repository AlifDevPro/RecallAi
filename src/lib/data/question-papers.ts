// Data-access layer for question papers.
// TODO(backend): replace fixtures with `GET /api/public/papers` filtered queries
// and `GET /api/public/papers/:id` for detail (returns digital + scans).
// The async getters below are the seam the backend phase will swap to real fetches.
...
export const ALL_DEPARTMENTS = Array.from(
  new Set(Object.values(DEPARTMENTS).flat()),
).sort();
...
function genScans(seed: number): ScanPage[] {
  const pages = (seed % 3) + 2; // 2-4 pages
  return Array.from({ length: pages }, (_, i) => ({
    // Placeholder scan page — backend will return signed URLs to R2/S3 objects.
...
export const PAPERS: Paper[] = Array.from({ length: 24 }, (_, i) => {
  ...
  const year = 2018 + (i % 7);
  const sem = ((i % 8) + 1);
  const hasPhoto = true; // every paper stores its original scan
  const hasDigital = i % 7 !== 6; // a few only-scans
